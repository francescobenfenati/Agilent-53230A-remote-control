...
def loadCsv(inputfilename, servername, user, password, dbname, metric,
    ...
    with inputfile as csvfile:
        ...
        for row in reader:
            print(row[timecolumn])
            #datetime_naive = datetime.datetime.strptime(row[timecolumn],timeformat)
            datetime_naive = datetime.datetime.strptime(datetime.datetime.fromtimestamp(float(row[timecolumn])).strftime(timeformat),timeformat)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-tf', '--timeformat', nargs='?', default='UTC',
